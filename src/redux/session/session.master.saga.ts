...
export function* sessionMasterSaga(): Generator {
  yield all([
    takeEvery(sessionActions.login.type, loginSaga),
    takeEvery(sessionActions.signUp.type, signUpSaga),
    takeEvery(sessionActions.refreshPage.type, refreshPageSaga),
    takeEvery(sessionActions.logout.type, logOutSaga),
  ]);
}   